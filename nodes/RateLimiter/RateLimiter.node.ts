...
export class RateLimiter implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const addMetadata = this.getNodeParameter('addMetadata', 0) as boolean;

		let delayMs: number;
		if (mode === 'fixedDelay') {
			delayMs = this.getNodeParameter('delay', 0) as number;
		} else if (mode === 'requestsPerSecond') {
			const rps = this.getNodeParameter('requestsPerSecond', 0) as number;
			delayMs = Math.floor(1000 / rps) * batchSize;
		} else {
			const rpm = this.getNodeParameter('requestsPerMinute', 0) as number;
